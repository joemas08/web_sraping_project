...
with open("frontpage.txt", "w") as file:
    ...
    frontpage_items_original_price = frontpage_grid.find_all(
        class_="dealCard__originalPrice"
    )

    file.write("https://slickdeals.net/\n\n")
    for title, price, og_price in zip(
        frontpage_items_title, frontpage_items_price, frontpage_items_original_price
    ):
        file.write(
            title.text
            + "\nSale Price: "
